...
import { UserService, PlanService, PlanToolService } from './src/lib/auth/services'
import { UserRole, ToolAccessType } from './src/lib/auth/types'
...
async function seedDatabase() {
  try {
    ...
    // Add tools to plans
    // Free plan gets limited access to some tools
    const freeTools = tools.slice(0, 30) // First 30 tools
    for (const tool of freeTools) {
      await PlanToolService.addToolToPlan(freePlan.id, tool.id, {
        accessType: ToolAccessType.LIMITED,
        maxUsage: 50,
        features: {}
      })
    }

    // Pro plan gets unlimited access to all tools
    for (const tool of tools) {
      await PlanToolService.addToolToPlan(proPlan.id, tool.id, {
    ...
    // Enterprise plan gets unlimited access to all tools with extra features
    for (const tool of tools) {
      await PlanToolService.addToolToPlan(enterprisePlan.id, tool.id, {
        accessType: ToolAccessType.UNLIMITED,
        maxUsage: -1,
        features: {
